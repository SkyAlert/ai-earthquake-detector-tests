...
class EventDataManager:
    """
    Gestor de eventos sísmicos con formato MiniSEED, JSON e imágenes
    """
    
    def __init__(self, station_name="SKYALERT", sampling_rate=100):
        self.station_name = station_name
        self.sampling_rate = sampling_rate
        self.pending_events = {}  # Eventos pendientes de completar
        self.waveform_buffer = {
            'ENZ': deque(maxlen=6000),  # 60 segundos de datos
            'ENE': deque(maxlen=6000),
            'ENN': deque(maxlen=6000),
            'timestamps': deque(maxlen=6000)
        }
        self.lock = threading.Lock()
        
        # Crear directorio de eventos
        self.events_dir = "events"
        if not os.path.exists(self.events_dir):
            os.makedirs(self.events_dir)
    
    def add_waveform_data(self, component, data, timestamp):
        """Añade datos de forma de onda al buffer circular"""
        with self.lock:
            if component in self.waveform_buffer:
                self.waveform_buffer[component].extend(data)
                # Añadir timestamps para cada muestra
                for i in range(len(data)):
                    sample_time = timestamp - (len(data) - i - 1) / self.sampling_rate
                    self.waveform_buffer['timestamps'].append(sample_time)
    
    def create_event_json(self, detection_result):
        """Crea archivo JSON inmediatamente al detectar anomalía"""
        try:
            ...
            event_data = {
                "event_id": event_id,
                "timestamp_detection": detection_time.isoformat(),
                "timestamp_real_start": real_event_start.isoformat(),
                "station": self.station_name,
                "confidence": float(detection_result['confidence']),
                "magnitude": float(detection_result['magnitude']) if detection_result['magnitude'] else None,
                "processing_id": detection_result['processing_id'],
                "status": "pending_waveform"
            }
            
            json_filename = os.path.join(self.events_dir, f"event_{event_id}.json")
            
            with open(json_filename, 'w') as f:
                json.dump(event_data, f, indent=2)
            
            # Programar generación de MiniSEED e imagen después de 1 minuto
            self.pending_events[event_id] = {
                'detection_result': detection_result,
                'json_file': json_filename,
                'real_start_time': real_event_start,
                'timer': threading.Timer(60.0, self._generate_waveform_files, args=[event_id])
            }
            
            self.pending_events[event_id]['timer'].start()
            
            logging.info(f"Evento JSON creado: {json_filename}")
            return event_id
            
        except Exception as e:
            logging.error(f"Error creando JSON del evento: {e}")
            return None
    
    def _generate_waveform_files(self, event_id):
        """Genera archivos MiniSEED e imagen después de 1 minuto"""
        try:
            if event_id not in self.pending_events:
                return
            
            event_info = self.pending_events[event_id]
            real_start_time = event_info['real_start_time']
            
            # Extraer 60 segundos de datos centrados en el evento
            with self.lock:
                waveform_data = self._extract_event_waveform(real_start_time)
            
            if waveform_data:
                # Generar MiniSEED
                miniseed_file = self._create_miniseed(event_id, waveform_data, real_start_time)
                
                # Generar imagen
                image_file = self._create_waveform_image(event_id, waveform_data, real_start_time)
                
                # Actualizar JSON con archivos generados
                self._update_event_json(event_info['json_file'], miniseed_file, image_file)
                
                logging.info(f"Archivos de evento completados: {event_id}")
            else:
                logging.warning(f"No hay suficientes datos para evento: {event_id}")
            
            # Limpiar evento pendiente
            del self.pending_events[event_id]
            
        except Exception as e:
            logging.error(f"Error generando archivos de forma de onda: {e}")
    
    def _extract_event_waveform(self, event_start_time):
        """Extrae 60 segundos de forma de onda centrados en el evento"""
        if len(self.waveform_buffer['timestamps']) < 6000:
            return None
        
        timestamps = list(self.waveform_buffer['timestamps'])
        enz_data = list(self.waveform_buffer['ENZ'])
        ene_data = list(self.waveform_buffer['ENE'])
        enn_data = list(self.waveform_buffer['ENN'])
        
        # Convertir event_start_time a timestamp
        event_timestamp = event_start_time.timestamp()
        
        # Encontrar índice más cercano al inicio del evento
        closest_idx = None
        min_diff = float('inf')
        
        for i, ts in enumerate(timestamps):
            diff = abs(ts - event_timestamp)
            if diff < min_diff:
                min_diff = diff
                closest_idx = i
        ...
        # Extraer 60 segundos (6000 muestras) centrados en el evento
        start_idx = max(0, closest_idx - 3000)
        end_idx = min(len(timestamps), start_idx + 6000)
        
        # Ajustar si no hay suficientes datos al final
        if end_idx - start_idx < 6000:
            start_idx = max(0, end_idx - 6000)
        
        return {
            'ENZ': enz_data[start_idx:end_idx],
            'ENE': ene_data[start_idx:end_idx],
            'ENN': enn_data[start_idx:end_idx],
            'timestamps': timestamps[start_idx:end_idx],
            'start_time': datetime.fromtimestamp(timestamps[start_idx])
        }
    
    def _create_miniseed(self, event_id, waveform_data, event_start_time):
        """Crea archivo MiniSEED"""
        try:
            miniseed_filename = os.path.join(self.events_dir, f"event_{event_id}.mseed")
            
            # Crear stream de ObsPy
            stream = Stream()
            
            components = ['ENZ', 'ENE', 'ENN']
            channels = ['HHZ', 'HHE', 'HHN']
            
            start_time_utc = UTCDateTime(waveform_data['start_time'])
            
            for comp, chan in zip(components, channels):
                if len(waveform_data[comp]) > 0:
                    # Crear stats para el trace
                    stats = Stats()
                    stats.network = "SK"
                    stats.station = self.station_name
                    stats.location = ""
                    stats.channel = chan
                    stats.starttime = start_time_utc
                    stats.sampling_rate = self.sampling_rate
                    stats.npts = len(waveform_data[comp])
                    
                    # Crear trace
                    trace = Trace(data=np.array(waveform_data[comp]), header=stats)
                    stream.append(trace)
            
            # Escribir MiniSEED
            stream.write(miniseed_filename, format='MSEED')
            logging.info(f"MiniSEED creado: {miniseed_filename}")
            
            return miniseed_filename
            
        except Exception as e:
            logging.error(f"Error creando MiniSEED: {e}")
            return None
    
    def _create_waveform_image(self, event_id, waveform_data, event_start_time):
        """Crea imagen PNG de la forma de onda"""
        try:
            image_filename = os.path.join(self.events_dir, f"event_{event_id}.png")
            
            # Crear timestamps relativos en segundos
            start_time = waveform_data['timestamps'][0]
            rel_times = [(ts - start_time) for ts in waveform_data['timestamps']]
            
            # Crear figura
            fig, axes = plt.subplots(3, 1, figsize=(12, 8))
            fig.suptitle(f'Evento Sísmico - {event_id}\nEstación: {self.station_name}', 
                        fontsize=14, fontweight='bold')
            
            components = [('ENZ', 'Vertical (Z)', COLOR_TEAL),
                         ('ENE', 'Este-Oeste (E)', COLOR_ORANGE),
                         ('ENN', 'Norte-Sur (N)', COLOR_RED)]
            
            for i, (comp, title, color) in enumerate(components):
                axes[i].plot(rel_times, waveform_data[comp], color=color, linewidth=0.8)
                axes[i].set_title(title, fontsize=12)
                axes[i].set_ylabel('Aceleración (Gals)', fontsize=10)
                axes[i].grid(True, alpha=0.3)
                axes[i].axhline(y=0, color='k', linestyle='-', alpha=0.3)
                
                # Marcar el momento de detección (10 segundos después del inicio)
                detection_time = 10.0
                axes[i].axvline(x=detection_time, color='red', linestyle='--', 
                              linewidth=2, alpha=0.7, label='Detección')
            ...
            logging.info(f"Imagen creada: {image_filename}")
            return image_filename
            
        except Exception as e:
            logging.error(f"Error creando imagen: {e}")
            return None
    
    def _update_event_json(self, json_file, miniseed_file, image_file):
        """Actualiza el archivo JSON con los archivos generados"""
        try:
            ...
            event_data['files'] = {
                'miniseed': os.path.basename(miniseed_file) if miniseed_file else None,
                'image': os.path.basename(image_file) if image_file else None
            }
            event_data['completion_time'] = datetime.now().isoformat()
            
            with open(json_file, 'w') as f:
                json.dump(event_data, f, indent=2)
                
        except Exception as e:
            logging.error(f"Error actualizando JSON: {e}")
    
    def cleanup_pending_events(self):
        """Limpia eventos pendientes al cerrar el sistema"""
        for event_id, event_info in self.pending_events.items():
            if 'timer' in event_info:
                event_info['timer'].cancel()
...
class TwoSecondLatencyDetector:
    ...
    def __init__(self, model_path, host='localhost', port=30000, sampling_rate=100):
        ...
        self.confidence_threshold = 0.09  # Menor umbral para mayor sensibilidad
        ...
        # Gestor de eventos
        self.event_manager = EventDataManager(station_name="SKYALERT", sampling_rate=sampling_rate)
    ...
    def save_event_data(self, detection_result):
        """Guarda datos del evento usando el nuevo sistema"""
        try:
            # Crear archivo JSON inmediatamente
            event_id = self.event_manager.create_event_json(detection_result)
            
            if event_id:
                logging.info(f"Evento registrado: {event_id} - MiniSEED e imagen se generarán en 1 minuto")
            else:
                logging.error("Error registrando evento")
                    ...
                    try:
                        ...
                        if packet_str:
                            ...
                            if parsed_data:
                                ...
                                # Añadir datos al gestor de eventos
                                self.event_manager.add_waveform_data(
                                    parsed_data['component'],
                                    parsed_data['data'],
                                    current_time
                                )
    ...
    def stop_system(self):
        ...
        self.event_manager.cleanup_pending_events()
